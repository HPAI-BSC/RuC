...
# // MODEL PARALLELISM //
TENSOR_PARALLEL_SIZE=4 # Split wihtin a node
PIPELINE_PARALLEL_SIZE=$SLURM_JOB_NUM_NODES # Split across nodes 
...
singularity exec --nv $SIF_PATH \
    bash -c "export VLLM_HOST_IP=$head_node_ip && export VLLM_CUDA_MEM_ALIGN_KV_CACHE=1 && vllm serve ${MODEL_PATH} \
        ...
        --max-model-len ${SEQUENCE_LENGTH_LIMIT} \
